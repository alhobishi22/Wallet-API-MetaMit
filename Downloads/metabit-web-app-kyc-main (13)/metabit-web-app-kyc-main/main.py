...
# القاموس لتخزين حالة المستخدمين
user_states = {}
...
def reset_user_state(user_id):
    ...
    :return: True إذا تم إعادة تعيين الحالة، False إذا لم يكن للمستخدم حالة
    """
    if user_id in user_states:
        prev_state = user_states.pop(user_id, None)
        logger.info(f"تم إعادة تعيين حالة المستخدم {user_id} (الحالة السابقة: {prev_state})")
        return True
    return False

def set_user_state(user_id, action, request_id, original_message_id=None, original_chat_id=None):
    """
    تعيين حالة المستخدم
    
    :param user_id: معرف المستخدم
    :param action: الإجراء (approve, reject)
    :param request_id: رقم الطلب
    :param original_message_id: معرف الرسالة الأصلية
    :param original_chat_id: معرف الدردشة الأصلية
    """
    user_states[user_id] = {
    ...
    logger.info(f"تم تعيين حالة المستخدم {user_id}: {action} للطلب {request_id}")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve_"):
        ...
        set_user_state(admin_id, "approve", request_id, query.message.message_id, query.message.chat_id)
        ...
        admin_id = update.effective_user.id
        set_user_state(admin_id, "reject", request_id, query.message.message_id, query.message.chat_id)
        ...
        try:
            # استخدام وظيفة قاعدة البيانات بدلاً من اتصال مباشر
            app_info = await database.get_application_info(request_id)
            
            if app_info:
                application_id = app_info.get("application_id")
                full_name = app_info.get("full_name")
                phone_number = app_info.get("phone_number")
                ...
                try:
                    ...
                    # إضافة تسجيل لتأكيد نجاح العملية
                    logger.info(f"تم الرجوع بنجاح لعرض طلب KYC رقم {request_id}")
                    ...
                    logger.info(f"تم إنشاء رسالة جديدة لعرض طلب KYC رقم {request_id} بعد فشل التحديث")
            else:
                try:
...
async def handle_admin_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"✉️ رسالة من المشرف {admin_full_name} (المعرف: {user_id}): {message_text}")
    ...
    if user_id in user_states and user_states[user_id]["awaiting_message"]:
        ...
        # إعادة تعيين حالة المستخدم قبل معالجة الإجراء لتجنب التداخل
                ...
                try:
                    ...
                    if app_info:
                        ...
                        try:
                            ...
                            logger.info(f"✅ تم تحديث رسالة الموافقة للطلب {request_id} بنجاح")
                        except Exception as edit_error:
                            logger.error(f"خطأ في تحديث رسالة الموافقة: {str(edit_error)}")
                            # إرسال رسالة جديدة كبديل
                            ...
                            if sent_message:
                                logger.info(f"✅ تم إرسال رسالة موافقة جديدة بعد فشل التحديث للطلب {request_id}")
                except Exception as e:
                    logger.error(f"خطأ في الحصول على معلومات الطلب للموافقة: {str(e)}")
                ...
                try:
                    ...
                    if app_info:
                        ...
                        confirmation_message = (
                            ...
                            f"*سبب الرفض:* {content}\n"
                            f"*بواسطة:* {admin_full_name}\n\n"
                            f"تم إرسال إشعار الرفض للمستخدم."
                        )
                        ...
                        try:
                            ...
                            logger.info(f"❌ تم تحديث رسالة الرفض للطلب {request_id} بنجاح")
                        except Exception as edit_error:
                            logger.error(f"خطأ في تحديث رسالة الرفض: {str(edit_error)}")
                            # إرسال رسالة جديدة كبديل
                            sent_message = await update.message.reply_text(
                                confirmation_message, 
                                parse_mode='Markdown'
                            ...
                            if sent_message:
                                logger.info(f"❌ تم إرسال رسالة رفض جديدة بعد فشل التحديث للطلب {request_id}")
                except Exception as e:
                    logger.error(f"خطأ في الحصول على معلومات الطلب للرفض: {str(e)}")
        
        # العملية انتهت، لا داعي لمتابعة معالجة الرسالة
        return
...
async def process_approval(update, request_id, registration_code, admin_full_name):
    """
    معالجة الموافقة على طلب التحقق من الهوية
    """
    try:
        logger.info(f"بدء معالجة الموافقة على الطلب {request_id} مع رمز التسجيل: {registration_code}")
        
        # تحديث حالة الطلب في قاعدة البيانات
        success, message = database.approve_application(request_id, admin_full_name, registration_code)
        
        if not success:
            logger.error(f"فشل في الموافقة على الطلب {request_id}: {message}")
            await update.message.reply_text(f"❌ {message}", parse_mode="Markdown")
            return False
        
        # الحصول على معرف التلغرام للمستخدم
        try:
            telegram_chat_id = await database.get_user_telegram_id(request_id)
            
            if telegram_chat_id:
                # إرسال إشعار للمستخدم
                notification_sent = await send_kyc_notification(
                    request_id, 
                    "approved", 
                    registration_code
                )
                if notification_sent:
                    logger.info(f"✅ تم إرسال إشعار الموافقة للمستخدم {telegram_chat_id}")
                else:
                    logger.warning(f"⚠️ فشل إرسال إشعار الموافقة للمستخدم {telegram_chat_id}")
            else:
                logger.warning(f"⚠️ لم يتم العثور على معرف تلغرام للطلب {request_id}")
        
        except Exception as e:
            logger.error(f"خطأ في إرسال إشعار الموافقة: {str(e)}")
            await update.message.reply_text(f"⚠️ تم الموافقة على الطلب ولكن فشل إرسال الإشعار: {str(e)}", parse_mode="Markdown")
            return False
        
        # إرسال رسالة تأكيد للمشرف
        await update.message.reply_text(f"✅ تمت الموافقة على الطلب {request_id} بنجاح", parse_mode="Markdown")
        logger.info(f"✅ تمت الموافقة بنجاح على الطلب {request_id} بواسطة {admin_full_name}")
        return True
        
    except Exception as e:
        logger.error(f"خطأ غير متوقع في معالجة الموافقة: {str(e)}")
        await update.message.reply_text(f"❌ حدث خطأ أثناء معالجة الموافقة: {str(e)}", parse_mode="Markdown")
        return False

async def process_rejection(update, request_id, reason, admin_full_name):
    """
    معالجة رفض طلب التحقق من الهوية
    """
    try:
        logger.info(f"بدء معالجة رفض الطلب {request_id} مع السبب: {reason}")
        
        # تحديث حالة الطلب في قاعدة البيانات
        success, message = database.reject_application(request_id, admin_full_name, reason)
        
        if not success:
            logger.error(f"فشل في رفض الطلب {request_id}: {message}")
            await update.message.reply_text(f"❌ {message}", parse_mode="Markdown")
            return False
        ...
        try:
            ...
            if telegram_chat_id:
                ...
                if notification_sent:
                    logger.info(f"❌ تم إرسال إشعار الرفض للمستخدم {telegram_chat_id}")
                else:
                    logger.warning(f"⚠️ فشل إرسال إشعار الرفض للمستخدم {telegram_chat_id}")
            else:
                logger.warning(f"⚠️ لم يتم العثور على معرف تلغرام للطلب {request_id}")
        
        except Exception as e:
            logger.error(f"خطأ في إرسال إشعار الرفض: {str(e)}")
            await update.message.reply_text(f"⚠️ تم رفض الطلب ولكن فشل إرسال الإشعار: {str(e)}", parse_mode="Markdown")
            return False
        
        # إرسال رسالة تأكيد للمشرف
        await update.message.reply_text(f"❌ تم رفض الطلب {request_id} بنجاح", parse_mode="Markdown")
        logger.info(f"❌ تم رفض الطلب {request_id} بنجاح بواسطة {admin_full_name}")
        return True
        
    except Exception as e:
        logger.error(f"خطأ غير متوقع في معالجة الرفض: {str(e)}")
        await update.message.reply_text(f"❌ حدث خطأ أثناء معالجة الرفض: {str(e)}", parse_mode="Markdown")
        return False
...
async def resend_notification(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        while attempt < max_attempts and not success:
            ...
            success = await send_telegram_message(telegram_chat_id, message, "Markdown")
