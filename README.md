...
- **تحليل رسائل متعددة**: دعم لرسائل SMS من محافظ متعددة (جيب، جوالي، كاش، الكريمي، ون كاش)
- **استخراج تلقائي**: استخراج تفاصيل المعاملات بشكل تلقائي من نصوص الرسائل
- **عرض مفصل**: ملخص للمعاملات حسب العملة مع تفاصيل كاملة
- **إدارة المعاملات**: إمكانية حذف المعاملات بشكل فردي
...
- **واجهة برمجة تطبيقات (API)**: واجهة REST API كاملة للوصول إلى البيانات من تطبيقات خارجية
...
- **استقبال مباشر للرسائل**: يمكن استقبال الرسائل مباشرة من تطبيق Forward SMS
- **اكتشاف ذكي للمحافظ**: تحليل محتوى الرسائل للتعرف تلقائيًا على نوع المحفظة
...
### استقبال الرسائل عبر Forward SMS

1. **إعداد تطبيق Forward SMS**: قم بتثبيت تطبيق Forward SMS على هاتفك
2. **تكوين التوجيه**: قم بضبط التطبيق ليرسل الرسائل إلى عنوان IP التطبيق الخاص بك على المنفذ 5000
3. **تحديد المسار**: استخدم المسار `/api/receive-sms` كنقطة نهاية للرسائل
4. **التحقق**: سيتم تحليل الرسائل تلقائياً وإضافتها إلى قاعدة البيانات
...
### بنك الكريمي (KuraimiIMB)
```
أودع/محمد احمد لحسابك757 SARرصيدك24246٫07SAR 
```
```
تم تحويل343٫83لحساب اسد الدين رصيدك866٫09USD 
```
...
```
```
حولت272,600.00لـأسامه عارف محمد
رسوم 0.00
رصيدك 0.00ر.ي
```
...
تم تصميم واجهة المستخدم بطريقة حديثة وسهلة الاستخدام، مع مراعاة الخصائص التالية:

- **توافق مع الهوية البصرية لـ MetaBit Analysis**
- **ألوان متدرجة**: استخدام تدرجات لونية من الأزرق إلى البنفسجي إلى البرتقالي
- **تمييز المحافظ**: استخدام ألوان مختلفة لكل محفظة (بنفسجي للكريمي، برتقالي لون كاش)
- **تمييز العملات**: تلوين مختلف لكل عملة لسهولة التمييز
- **تصميم متجاوب**: يعمل على جميع أحجام الشاشات من الهواتف إلى أجهزة الكمبيوتر المكتبية
- **تفاعلية**: أزرار وعناصر تفاعلية مع تأثيرات حركية بسيطة لتحسين تجربة المستخدم

## واجهة برمجة التطبيقات (API)

يوفر التطبيق واجهة برمجية RESTful API تتيح للتطبيقات الخارجية الوصول إلى بيانات المعاملات. تشمل الميزات:

### نقاط النهاية المتاحة

- `GET /api/wallets`: الحصول على قائمة المحافظ المتاحة
- `GET /api/transactions`: البحث والتصفية في جميع المعاملات
- `GET /api/wallets/{wallet_name}/transactions`: الحصول على معاملات محفظة محددة
- `GET /api/wallets/{wallet_name}/summary`: الحصول على ملخص مالي لمحفظة محددة

### الأمان والمصادقة

تستخدم واجهة API مفاتيح API للمصادقة. يجب إرسال المفتاح في رأس الطلب:

```
X-API-Key: MetaBit_API_Key_24X7
```

لمزيد من المعلومات حول استخدام API، قم بزيارة صفحة التوثيق على `/api/docs`.

## قاعدة البيانات

يستخدم التطبيق قاعدة بيانات PostgreSQL المستضافة على Render:

- **السرعة والأداء**: قاعدة بيانات متقدمة تتعامل مع كميات كبيرة من البيانات
- **متزامنة**: تسمح بعدة اتصالات في وقت واحد
- **مستضافة سحابيًا**: متاحة دائمًا ومن أي موقع
- **آمنة**: تتضمن ميزات حماية البيانات والنسخ الاحتياطي
